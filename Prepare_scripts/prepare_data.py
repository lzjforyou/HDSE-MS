...
from data_utils import list_str2float, booltype, tqdm_joblib
...
def load_df(df_dp, dset_names, num_entries):
    ...
    for dset_name in dset_names:
        ...
        print("Columns:", list(dset_df.columns))
        dfs.append(dset_df)
    if num_entries > 0:
        dfs = [
            df.sample(
...
def preprocess_spec(spec_df):
    ...
    spec_df = spec_df[["spec_id", "mol_id", "prec_type", "inst_type", "frag_mode", "spec_type", "ion_mode", "dset", "col_gas", "res", "ace", "nce", "prec_mz", "peaks", "ri"]]
    # relabel spec_id to make it unique across datasets
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dset_names", type=str, default="nist_23_hr_records,mb_na,mona,massbank")
    ...
    spec_df_fp = os.path.join(data_dp, "spec_23_df.pkl")
    mol_df_fp = os.path.join(data_dp, "mol_23_df.pkl")
    ...
    smiles_df.to_csv(os.path.join(data_dp,"all_smiles.txt"),sep=" ",header=False,index=False)
