...
from data_utils import par_apply_series, check_mol_props
from data_utils import none_or_nan
from casmi_utils import common_filter, load_mw_cand, prepare_casmi_mol_df, prepare_casmi_cand_df, prepare_casmi_spec_df
...
def main(args):
    ...
    sol_df.loc[:, "spec_id"] = sol_df["spec_id"].str[id_offset:id_offset + 3].astype(int)
    ...
    for cand_idx, cand_fp in enumerate(cand_fps):
        ...
        query_spec_ids.extend([spec_ids[cand_idx] for i in range(cand_df.shape[0])])
        cand_smileses.append(sol_df[sol_df["spec_id"] == spec_ids[cand_idx]]["smiles"].item())
        query_spec_ids.append(spec_ids[cand_idx])
    cand_df = pd.DataFrame({"candidate_smiles": cand_smileses, "query_spec_id": query_spec_ids})
    ...
    cand_df.loc[:, "candidate_mol_id"] = cand_df["candidate_smiles"].map(smiles_to_id)
    ...
    spec_df.loc[:, "ace"] = np.nan
    spec_df.loc[:, "nce"] = np.nan
    ...
    os.makedirs(os.path.join(args.proc_dp, args.casmi_output_dir), exist_ok=True)
    spec_df_fp = os.path.join(args.proc_dp, args.casmi_output_dir, "spec_df.pkl")
    mol_df_fp = os.path.join(args.proc_dp, args.casmi_output_dir, "mol_df.pkl")
    cand_df_fp = os.path.join(args.proc_dp, args.casmi_output_dir, "cand_df.pkl")
    ...
    export_mol_df.to_csv(
        os.path.join(args.proc_dp, args.casmi_output_dir, "all_smiles.txt"),
        sep=" ",
        header=False,
        index=False)
